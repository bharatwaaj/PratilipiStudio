...
/**
 * Helper for controlling the visibility of the System UI across the various API
 * levels. To use this API, instantiate an instance of this class with the
 * required level. The level specifies the extent to which the System UI's
 * visibility is changed when you call {@link #hide()} or {@link #toggle()}.
 */
public final class SystemUiHelper {
	...
	/**
	 * In this level, the helper will toggle the visibility of the navigation
	 * bar (if present and if possible) and status bar. In cases where the
	 * navigation bar is present but cannot be hidden, it will toggle low
	 * profile mode.
	 */
	...
	/**
	 * In this level, the helper will toggle the visibility of the navigation
	 * bar (if present and if possible) and status bar, in an immersive mode.
	 * This means that the app will continue to receive all touch events. The
	 * user can reveal the system bars with an inward swipe along the region
	 * where the system bars normally appear.
	 * 
	 * <p>
	 * The {@link #FLAG_IMMERSIVE_STICKY} flag can be used to control how the
	 * system bars are displayed.
	 */
	...
	/**
	 * Used with {@link #LEVEL_IMMERSIVE}. When this flag is set, an inward
	 * swipe in the system bars areas will cause the system bars to temporarily
	 * appear in a semi-transparent state, but no flags are cleared, and your
	 * system UI visibility change listeners are not triggered. The bars
	 * automatically hide again after a short delay, or if the user interacts
	 ...
	 * 
	 * @param activity
	 *            The Activity who's system UI should be changed
	 * @param level
	 *            The level of hiding. Should be either
	 *            {@link #LEVEL_LOW_PROFILE}, {@link #LEVEL_HIDE_STATUS_BAR},
	 *            {@link #LEVEL_LEAN_BACK} or {@link #LEVEL_IMMERSIVE}
	 * @param flags
	 *            Additional options. See
	 *            {@link #FLAG_LAYOUT_IN_SCREEN_OLDER_DEVICES} and
	 ...
	 *            {@link #FLAG_IMMERSIVE_STICKY}
	 * @param listener
	 *            A listener which is called when the system visibility is
	 *            changed
	 */
	public SystemUiHelper(Activity activity, int level, int flags,
	...
	/**
	 * @return true if the system UI is currently showing. What this means
	 *         depends on the mode this
	 *         {@link android.example.android.systemuivis.SystemUiHelper} was
	 *         instantiated with.
	 */
	...
	/**
	 * Show the system UI. What this means depends on the mode this
	 * {@link android.example.android.systemuivis.SystemUiHelper} was
	 * instantiated with.
	 * 
	 * <p>
	...
	/**
	 * Hide the system UI. What this means depends on the mode this
	 * {@link android.example.android.systemuivis.SystemUiHelper} was
	 * instantiated with.
	 * 
	 * <p>
